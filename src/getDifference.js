...
const buidTree = (data1, data2) => {
  ...
  const diff = keys.map((key) => {
    ...
    if (!_.has(data1, key)) {
      result = { type: 'added', key, value: data2[key] };
    }
    if (!_.has(data2, key)) {
      result = { type: 'removed', key, value: data1[key] };
    }
    if (_.isPlainObject(data1[key]) && _.isPlainObject(data2[key])) {
      result = { type: 'nested', key, children: buidTree(data1[key], data2[key]) };
    }
    if (_.isEqual(data1[key], data2[key])) {
      result = { type: 'unchanged', key, value: data1[key] };
    }
    return result;
  });
